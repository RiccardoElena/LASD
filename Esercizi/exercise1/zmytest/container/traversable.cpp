...
// Auxiliary Function to be executed by High Order Functions (As Traverse, Fold, Map, ...)
...
std::string FoldStringConcatenateInitials(const std::string &dat, const std::string &acc)
{
  std::string newstr = acc;
  if (!dat.empty())
  {
...
std::string FoldStringConcatenateGraficInitials(const std::string &dat, const std::string &acc)
{
  std::string newstr = acc;
  char initial = dat[0];
...
std::string FoldDigitsConcatenate(const std::string &dat, const std::string &acc)
{
  std::string newstr = acc;
  for (char c : dat)
  {
...
std::string FoldLowercaseConcatenate(const std::string &dat, const std::string &acc)
{
  std::string newstr = acc;
  for (char c : dat)
  {
...
std::string FoldUppercaseConcatenate(const std::string &dat, const std::string &acc)
{
  std::string newstr = acc;
  for (char c : dat)
  {
...
std::string FoldWhitespaceConcatenate(const std::string &dat, const std::string &acc)
{
  std::string newstr = acc;
  for (char c : dat)
  {
...
std::string FoldVowelsConcatenate(const std::string &dat, const std::string &acc)
{
  std::string newstr = acc;
  for (char c : dat)
  {
...
std::string FoldConsonantsConcatenate(const std::string &dat, const std::string &acc)
{
  std::string newstr = acc;
  for (char c : dat)
  {
...
{
  return acc + (person.getGender() == Gender::M ? 1 : person.getGender() == Gender::F ? -1
                                                                                      : 0);
}

std::string FoldConcatenateFullName(const Person &person, const std::string &acc)
{
  std::string newstr = acc;
  if (!newstr.empty())
  {
...
template <typename Data>
void MapMultiplyByTwo(Data &dat)
{
  dat *= 2;
}

template <typename Data>
void MapIncreaseByOne(Data &dat)
{
  dat += 1;
}

template <typename Data>
void MapDecreaseByOne(Data &dat)
{
  dat -= 1;
}
...
{
  std::transform(person.getFiscalCode().begin(), person.getFiscalCode().end(), person.getFiscalCode().begin(), ::toupper);
}
