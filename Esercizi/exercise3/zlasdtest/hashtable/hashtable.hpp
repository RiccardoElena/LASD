...
void EqualHT(uint &testnum, uint &testerr, const HT &ht1, const HT &ht2) {
  ...
  try {
    std::cout << " " << testnum << " (" << testerr
              << ") The two hash tables are "
              << ((tst = (ht1 == ht2)) ? "" : "not ") << "equal: ";
...
void CountHT(uint &testnum, uint &testerr, const lasd::HashTable<Data> &ht,
             const lasd::LinearContainer<Data> &con, uint num) {
  testnum++;
  bool tst = true;
  uint cnt = 0;
  try {
    ...
    std::cout << " " << testnum << " (" << testerr
              << ") The hash table contains " << cnt
              << " elements of the linear container: ";
    std::cout << ((tst = (cnt == num)) ? "Correct" : "Error") << "!"
              << std::endl;
  } catch (std::exception &exc) {
    std::cout << "\"" << exc.what() << "\": " << "Error!" << std::endl;
  }
